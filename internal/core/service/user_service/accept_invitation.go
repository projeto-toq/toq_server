...
func (us *userService) AcceptInvitation(ctx context.Context, userID int64) (err error) {
	...
	if err != nil {
		logger.Error("user.accept_invitation.tx_start_error", "err", err)
	...
	defer func() {
		if err != nil {
			if rbErr := us.globalService.RollbackTransaction(ctx, tx); rbErr != nil {
				logger.Error("user.accept_invitation.tx_rollback_error", "err", rbErr)
	...
	if err != nil {
		logger.Error("user.accept_invitation.tx_commit_error", "err", err)
		utils.SetSpanError(ctx, err)
...
func (us *userService) acceptInvitation(ctx context.Context, tx *sql.Tx, userID int64) (relationshipID int64, err error) {
	ctx = utils.ContextWithLogger(ctx)
	logger := utils.LoggerFromContext(ctx)
	//recover the realtor
	...
	if err != nil {
		utils.SetSpanError(ctx, err)
		logger.Error("user.accept_invitation.repo_get_user_by_id_error", "user_id", userID, "err", err)
	...
	if err != nil {
		...
		utils.SetSpanError(ctx, err)
		logger.Error("user.accept_invitation.repo_get_invite_error", "phone_number", realtor.GetPhoneNumber(), "err", err)
	...
	if err != nil {
		utils.SetSpanError(ctx, err)
		logger.Error("user.accept_invitation.repo_get_agency_error", "agency_id", invite.GetAgencyID(), "err", err)
		return 0, utils.InternalError("Failed to get agency")
	}

	//create the agency <-> realtor relationship
	relationshipID, err = us.repo.CreateAgencyRelationship(ctx, tx, agency, realtor)
	if err != nil {
		utils.SetSpanError(ctx, err)
		logger.Error("user.accept_invitation.repo_create_agency_relationship_error", "agency_id", agency.GetID(), "realtor_id", realtor.GetID(), "err", err)
	...
	if err != nil {
		utils.SetSpanError(ctx, err)
		logger.Error("user.accept_invitation.repo_delete_invite_error", "invite_id", invite.GetID(), "err", err)
	...
	if activeRole == nil || activeRole.GetRole() == nil {
		errActiveRole := utils.InternalError("Active role missing")
		logger.Error("user.active_role.missing", "user_id", realtor.GetID())
		utils.SetSpanError(ctx, errActiveRole)
		return 0, errActiveRole
	}
	...
	if err != nil {
		utils.SetSpanError(ctx, err)
		logger.Error("user.accept_invitation.update_user_status_error", "user_id", realtor.GetID(), "role", string(roleSlug), "err", err)
	...
	if err != nil {
		utils.SetSpanError(ctx, err)
		logger.Error("user.accept_invitation.notification_send_error", "agency_id", agency.GetID(), "err", err)
	...
	if auditErr := us.auditService.RecordChange(ctx, tx, auditRecord); auditErr != nil {
		...
		logger.Error("user.accept_invitation.audit_create_error", "invite_id", invite.GetID(), "err", auditErr)
