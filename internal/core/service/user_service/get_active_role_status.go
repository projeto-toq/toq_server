...
func (us *userService) GetActiveRoleStatus(ctx context.Context) (status permissionmodel.UserRoleStatus, err error) {
	...
	if txErr != nil {
		...
		logger.Error("user.get_active_role_status.tx_start_error", "error", txErr)
	...
	defer func() {
		if err != nil {
			if rbErr := us.globalService.RollbackTransaction(ctx, tx); rbErr != nil {
				...
				logger.Error("user.get_active_role_status.tx_rollback_error", "error", rbErr)
			}
		}
	}()
	...
	if activeRole == nil {
		...
		logger.Error("user.get_active_role_status.active_role_missing", "user_id", userID)
		return 0, utils.InternalError("Active role missing")
	}

	if cmErr := us.globalService.CommitTransaction(ctx, tx); cmErr != nil {
		utils.SetSpanError(ctx, cmErr)
		logger.Error("user.get_active_role_status.tx_commit_error", "error", cmErr)
