...
func (p *permissionServiceImpl) GetUserPermissions(ctx context.Context, userID int64) ([]permissionmodel.PermissionInterface, error) {
	...
	logger.Debug("permission.user.permissions.fetch.start", "user_id", userID)

	// Start transaction
	tx, err := p.globalService.StartTransaction(ctx)
	if err != nil {
		logger.Error("permission.user.permissions.tx_start_failed", "user_id", userID, "error", err)
	...
	if err != nil {
		logger.Error("permission.user.permissions.db_failed", "user_id", userID, "error", err)
		utils.SetSpanError(ctx, err)
		return nil, utils.InternalError("")
	}
	...
	if err = p.globalService.CommitTransaction(ctx, tx); err != nil {
		logger.Error("permission.user.permissions.tx_commit_failed", "user_id", userID, "error", err)
		utils.SetSpanError(ctx, err)
		return nil, utils.InternalError("")
	}

	logger.Info("permission.user.permissions.fetched", "user_id", userID, "count", len(permissions))
