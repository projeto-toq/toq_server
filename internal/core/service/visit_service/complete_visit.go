...
func (s *visitService) CompleteVisit(ctx context.Context, visitID int64, ownerNotes string) (listingmodel.VisitInterface, error) {
	...
	if txErr != nil {
		...
		logger.Error("visit.complete.tx_start_error", "err", txErr)
	...
	defer func() {
		if !committed {
			if rbErr := s.globalService.RollbackTransaction(ctx, tx); rbErr != nil {
				...
				logger.Error("visit.complete.tx_rollback_error", "err", rbErr)
			}
		}
	}()
	...
	if err = s.visitRepo.UpdateVisit(ctx, tx, visit); err != nil {
		...
		logger.Error("visit.complete.update_visit_error", "visit_id", visitID, "err", err)
		return nil, utils.InternalError("")
	}
	...
	if commitErr := s.globalService.CommitTransaction(ctx, tx); commitErr != nil {
		...
		logger.Error("visit.complete.tx_commit_error", "err", commitErr)
		return nil, utils.InternalError("")
	}
	committed = true
