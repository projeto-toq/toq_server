...
func parseScheduleRange(rangeReq dto.ScheduleRangeRequest) (schedulemodel.ScheduleRange, error) {
	...
	if rangeReq.From != "" {
		from, err := utils.ParseRFC3339Relaxed("range.from", rangeReq.From)
		if err != nil {
			return schedulemodel.ScheduleRange{}, err
		}
		result.From = from
	}
	if rangeReq.To != "" {
		to, err := utils.ParseRFC3339Relaxed("range.to", rangeReq.To)
		if err != nil {
			return schedulemodel.ScheduleRange{}, err
		}
		result.To = to
	}
	result.Loc = utils.DetermineRangeLocation(result.From, result.To, nil)
	return result, nil
}
