...
func (h *AdminHandler) PutAdminUpdateComplexZipCode(c *gin.Context) {
	...
	input := propertycoverageservice.UpdateComplexZipCodeInput{
		ID: req.ID,
		CreateComplexZipCodeInput: propertycoverageservice.CreateComplexZipCodeInput{
			HorizontalComplexID: req.ComplexID,
			ZipCode:             req.ZipCode,
		},
	}

	zipCode, err := h.propertyCoverageService.UpdateComplexZipCode(ctx, input)
	if err != nil {
