...
func (uh *UserHandler) ResendPhoneChangeCode(c *gin.Context) {
	...
	code, err := uh.userService.ResendPhoneChangeCode(ctx, userInfo.ID)
	if err != nil {
		utils.SendHTTPError(c, http.StatusInternalServerError, "RESEND_PHONE_CHANGE_CODE_FAILED", "Failed to resend phone change code")
		return
	}

	// Prepare response
	response := dto.ResendPhoneChangeCodeResponse{
		Code: code,
	}
