...
// ListSlotsForPeriod returns all slots in a range disregarding photographer filtering.
func (a *PhotoSessionAdapter) ListSlotsForPeriod(ctx context.Context, tx *sql.Tx, rangeStart, rangeEnd time.Time) ([]photosessionmodel.PhotographerSlotInterface, error) {
	...
	rows, execErr := tx.QueryContext(ctx, query, rangeEnd, rangeStart)
	if execErr != nil {
		utils.SetSpanError(ctx, execErr)
		logger.Error("mysql.photo_session.list_slots_for_period.query_error", "err", execErr)
		return nil, fmt.Errorf("list photographer slots for period: %w", execErr)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(
			...
			logger.Error("mysql.photo_session.list_slots_for_period.scan_error", "err", err)
	...
	if err = rows.Err(); err != nil {
		utils.SetSpanError(ctx, err)
		logger.Error("mysql.photo_session.list_slots_for_period.rows_error", "err", err)
