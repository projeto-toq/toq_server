...
func SessionEntityToDomain(entity []any) (session sessionmodel.SessionInterface, err error) {
	...
	if !ok {
		...
		return nil, status.Error(codes.Internal, "Error converting id to int64")
	}
	session.SetID(id)
	...
	if !ok {
		...
		return nil, status.Error(codes.Internal, "Error converting refresh_hash to []byte")
	}
	...
	if entity[3] != nil {
		...
		if !ok {
			...
			return nil, status.Error(codes.Internal, "Error converting token_jti to []byte")
		}
	...
	if !ok {
		...
		return nil, status.Error(codes.Internal, "Error converting expires_at to time.Time")
	}
	...
	if entity[5] != nil {
		...
		if !ok {
			...
			return nil, status.Error(codes.Internal, "Error converting absolute_expires_at to time.Time")
		}
	...
	if !ok {
		...
		return nil, status.Error(codes.Internal, "Error converting created_at to time.Time")
	}
	...
	if entity[7] != nil {
		...
		if !ok {
			...
			return nil, status.Error(codes.Internal, "Error converting rotated_at to time.Time")
		}
	...
	if entity[8] != nil {
		...
		if !ok {
			...
			return nil, status.Error(codes.Internal, "Error converting user_agent to []byte")
		}
	...
	if entity[9] != nil {
		...
		if !ok {
			...
			return nil, status.Error(codes.Internal, "Error converting ip to []byte")
		}
	...
	if entity[10] != nil {
		...
		if !ok {
			...
			return nil, status.Error(codes.Internal, "Error converting device_id to []byte")
		}
	...
	if entity[11] != nil {
		...
		if !ok {
			...
			return nil, status.Error(codes.Internal, "Error converting rotation_counter to int64")
		}
	...
	if entity[12] != nil {
		...
		if !ok {
			...
			return nil, status.Error(codes.Internal, "Error converting last_refresh_at to time.Time")
		}
	...
	if !ok {
		...
		return nil, status.Error(codes.Internal, "Error converting revoked to bool")
	}
