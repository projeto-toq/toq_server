...
func (a *MediaProcessingAdapter) GetAsset(ctx context.Context, tx *sql.Tx, listingIdentityID uint64, assetType mediaprocessingmodel.MediaAssetType, sequence uint8) (mediaprocessingmodel.MediaAsset, error) {
	var entity mediaprocessingentities.AssetEntity
	err := a.QueryRowContext(ctx, tx, "get_asset", getAssetQuery, listingIdentityID, string(assetType), sequence).Scan(
		&entity.ID,
