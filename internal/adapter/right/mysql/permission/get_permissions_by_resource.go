...
func (pa *PermissionAdapter) GetPermissionsByResource(ctx context.Context, tx *sql.Tx, resource string) (permissions []permissionmodel.PermissionInterface, err error) {
	...
	if err != nil {
		...
		logger.Error("mysql.permission.get_permissions_by_resource.read_error", "error", err)
		return nil, fmt.Errorf("get permissions by resource read: %w", err)
	}
	...
	for index, row := range results {
		if len(row) != 8 {
			...
			logger.Error("mysql.permission.get_permissions_by_resource.columns_mismatch", "row_index", index, "error", errColumns)
		...
		if convertErr != nil {
			...
			logger.Error("mysql.permission.get_permissions_by_resource.convert_error", "row_index", index, "error", convertErr)
	...
	logger.Debug("mysql.permission.get_permissions_by_resource.success", "count", len(permissions))
