...
func (a *HolidayAdapter) CreateCalendar(ctx context.Context, tx *sql.Tx, calendar holidaymodel.CalendarInterface) (uint64, error) {
	...
	query := `INSERT INTO holiday_calendars (name, scope, state, city_ibge, is_active, created_by, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?)`
	result, err := exec.ExecContext(ctx, query, entity.Name, entity.Scope, entity.State, entity.CityIBGE, entity.IsActive, entity.CreatedBy, entity.UpdatedBy)
	if err != nil {
