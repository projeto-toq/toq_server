...
func (a *HolidayAdapter) GetCalendarByID(ctx context.Context, tx *sql.Tx, id uint64) (holidaymodel.CalendarInterface, error) {
	...
	query := `SELECT id, name, scope, state, city_ibge, is_active FROM holiday_calendars WHERE id = ?`
	row := exec.QueryRowContext(ctx, query, id)

	var calendarEntity entity.CalendarEntity
	if err = row.Scan(&calendarEntity.ID, &calendarEntity.Name, &calendarEntity.Scope, &calendarEntity.State, &calendarEntity.CityIBGE, &calendarEntity.IsActive); err != nil {
