...
func (a *HolidayAdapter) UpdateCalendar(ctx context.Context, tx *sql.Tx, calendar holidaymodel.CalendarInterface) error {
	...
	query := `UPDATE holiday_calendars SET name = ?, scope = ?, state = ?, city_ibge = ?, is_active = ?, updated_by = ? WHERE id = ?`
	result, err := exec.ExecContext(ctx, query, entity.Name, entity.Scope, entity.State, entity.CityIBGE, entity.IsActive, entity.UpdatedBy, entity.ID)
	if err != nil {
